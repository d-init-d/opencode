...
OpenCode includes **CLIProxyAPI** as a built-in provider for GPT and Codex-style models.

1. Start your local CLIProxyAPI service, or copy the hosted endpoint you want to use.

2. Run the `/connect` command and search for **CLIProxyAPI**.

   ```txt
   /connect
   ```

3. Enter the endpoint you want OpenCode to use.
...
5. Run `/models` and pick one of the available GPT or Codex models.

   ```txt
   /models
   ```

If you want to set or change the endpoint manually, add it to your config:
...
Re-running `/connect` for CLIProxyAPI updates both the stored API key and the configured endpoint.
